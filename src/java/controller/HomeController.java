...
public class HomeController extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest req, HttpServletResponse response)
        ...
        // Product giả tưởng
        List<Product> giatuong = productService.getProductById(3);
        req.setAttribute("giatuong", giatuong);

        // Product truyện tranh
        List<Product> truyentranh = productService.getProductById(1);
        req.setAttribute("truyentranh", truyentranh);
        // Check kinh dị
        List<Product> trinhtham = productService.getProductById(4);
        req.setAttribute("trinhtham", trinhtham);

        // Check tiểu thuyết
        List<Product> tieuthuyet = productService.getProductById(2);
        req.setAttribute("tieuthuyet", tieuthuyet);
        
        // Product ngôn tình
        List<Product> ngontinh = productService.getProductById(5);
        req.setAttribute("ngontinh", ngontinh);
        
        // Product kinh điển
        List<Product> kinhdien = productService.getProductById(6);
        req.setAttribute("kinhdien", kinhdien);
